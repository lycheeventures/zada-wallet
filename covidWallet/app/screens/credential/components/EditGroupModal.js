...
const EditGroupModal = ({
  ...
  useEffect(() => {
    const _changeCreds = () => {
      let notSelectedCreds = [],
        selectedCreds = [];
      credentials.forEach((item, index) => {
        let selected = groupCredentials.find(
          (groupCred) => groupCred.credentialId == item.credentialId
        );
        if (selected) {
          let obj = {
            ...item,
  ...
  return (
    <Modal
      ...
      <SafeAreaView style={styles._mainContainer}>
        <HeadingComponent text={`Edit Group`} />
          ...
          <FlatList
            ...
            renderItem={({ item, index }) => (
              <Pressable
                ...
                onPress={() => {
                  let credIndex = creds.findIndex(
                    (c) => c.credentialId === item.credentialId
                  );
