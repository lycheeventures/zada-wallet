...
export const acceptMultipleConnection = createAsyncThunk(
  'connection/acceptMultipleConnection',
  async (metadata: string[]) => {
    try {
      let response = await ConnectionAPI.accept_multiple_connection(metadata);
      return response.data;
    } catch (e: any) {
      throw e;
    }
